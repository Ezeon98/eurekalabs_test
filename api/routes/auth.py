...
def register(user: SignupData, request: Request):
    ...
    It receives a SignupData object in the request body, generates a new API key using
    the generate_api_key function, associates it with the user's name,
    and returns the API key in a dictionary format.
